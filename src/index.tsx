...
function addSkillField(s=null){const c=document.getElementById('skillsContainer');const d=document.createElement('div');d.className='flex space-x-2 items-center skill-field';d.innerHTML=\`<input type="text" name="skill_name[]" value="\${s?.name||''}" placeholder="Compétence" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm" list="skills-datalist" /><input type="text" name="skill_category[]" value="\${s?.category||''}" placeholder="Catégorie" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm" /><button type="button" onclick="this.parentElement.remove()" class="text-red-600 hover:text-red-800"><i class="fas fa-times"></i></button>\`;c.insertBefore(d,c.lastElementChild);if(!document.getElementById('skills-datalist')){const dl=document.createElement('datalist');dl.id='skills-datalist';allSkills.forEach(s=>{const o=document.createElement('option');o.value=s.name;dl.appendChild(o);});document.body.appendChild(dl);}}
document.getElementById('experienceForm').addEventListener('submit',async(e)=>{e.preventDefault();const id=document.getElementById('experience_id').value;const fd={company:document.getElementById('company').value,position:document.getElementById('position').value,location:document.getElementById('location').value,start_date:document.getElementById('start_date').value,end_date:document.getElementById('end_date').value,is_current:document.getElementById('is_current').checked,description:document.getElementById('description').value,achievements:document.getElementById('achievements').value,skills:[]};const sn=document.getElementsByName('skill_name[]');const sc=document.getElementsByName('skill_category[]');for(let i=0;i<sn.length;i++){if(sn[i].value.trim()){fd.skills.push({name:sn[i].value.trim(),category:sc[i].value.trim()||'Other'});}}try{if(id){await axios.put(\`/api/experiences/\${id}\`,fd);}else{await axios.post('/api/experiences',fd);}closeModal();await loadExperiences();await loadSkills();}catch(e){console.error(e);alert('Erreur d\\'enregistrement');}});
